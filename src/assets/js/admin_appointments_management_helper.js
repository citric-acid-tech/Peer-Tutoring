...
//			$('#filter-appointments_management .curtain').slideDown(360);
    ...
    AdminAppointmentsManagementHelper.prototype.display = function (appointment) {
		...
		if (appointment.attachment_url !== 'no_file') {
			$('#download a').prop('href', GlobalVariables.downloadPrefix + appointment.attachment_url + "/" + appointment.service_type.replace(' ', '') + "/" + (appointment.student_sid === null ? "0" : appointment.student_sid) + "/" + (appointment.tutor_sid === null ? "0" : appointment.tutor_sid) + "/" + date);
